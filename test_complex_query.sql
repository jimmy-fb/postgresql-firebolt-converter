select
	...
	array_agg(distinct c."lincoln-broker-office")
	filter (where c."lincoln-broker-office" is not null) as "lincoln-broker-office",

	array_agg(distinct c."lincoln-broker-region")
	filter (where c."lincoln-broker-region" is not null) as "lincoln-broker-region",
	...
	array_agg(distinct c."full-state")
	filter (where c."full-state" is not null)                                    as all_plan_states,
	public.array_union(c.products)
	filter (where c.products is not null)                                        as all_plan_products,
	array_agg(distinct c."carrier-name")
	filter (where c."carrier-name" is not null)                                  as all_carriers,
  ...
  (
	...
	from (select
		  ...
		  from insurance_plans_core_brokers            as cc_inner
			  ...
			  join agents                              as ag
				on ag.npn = (cj_ag.agent ->> 'npn')::integer
   ) as cc
