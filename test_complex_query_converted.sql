select
	...
	array_agg(CASE WHEN c."lincoln-broker-office" is not null THEN distinct c."lincoln-broker-office" ELSE 0 END) as "lincoln-broker-office",

	array_agg(CASE WHEN c."lincoln-broker-region" is not null THEN distinct c."lincoln-broker-region" ELSE 0 END) as "lincoln-broker-region",
	...
	array_agg(CASE WHEN c."full-state" is not null THEN distinct c."full-state" ELSE 0 END)                                    as all_plan_states,
	public.array_union(CASE WHEN c.products is not null THEN c.products ELSE 0 END)                                        as all_plan_products,
	array_agg(CASE WHEN c."carrier-name" is not null THEN distinct c."carrier-name" ELSE 0 END)                                  as all_carriers,
